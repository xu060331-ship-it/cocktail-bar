      ...
      <div className="max-w-4xl mx-auto px-5">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} className="mb-10">
          <div className="flex items-center justify-between mb-8">
            ...
            <button onClick={logout} className="flex items-center gap-1.5 text-sm text-[var(--color-text-gray)] hover:text-white transition-colors border border-[var(--color-border)] rounded-full px-4 py-2">
          ...
          <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            <div className="flex gap-4 mb-6">
              {[{ key: "favorites", label: "收藏", icon: Heart }, { key: "history", label: "浏览记录", icon: Clock }].map(t => (
                <button key={t.key} onClick={() => { setActiveTab(t.key); setMatchResult(null) }}
                  className={`text-sm flex items-center gap-1.5 pb-2 border-b-2 transition-colors ${activeTab === t.key ? "text-white border-[var(--color-accent)]" : "text-[var(--color-text-muted)] border-transparent hover:text-white"}`}>
                  ...
                  <motion.div key={c.eng} initial={{ opacity: 0, x: -16 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}>
                    <Link to={`/cocktails/${encodeURIComponent(c.eng)}`} className="flex items-center gap-4 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-4 hover:border-[var(--color-accent)] transition-all group">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm text-white font-serif group-hover:text-[var(--color-accent)] transition-colors">{c.chn?.replace(/[（(][^）)]*[）)]/g, "").trim() || c.eng}</h3>
        ...
        {activeTab === "bar" && (
          <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            <h2 className="text-lg text-white font-serif mb-2 flex items-center gap-2"><GlassWater size={16} className="text-[var(--color-accent)]" /> 我的吧台</h2>
                    ...
                    <div className="space-y-2">
                      {matchResult.matchable.map((c, i) => (
                        <Link key={c.eng} to={`/cocktails/${encodeURIComponent(c.eng)}`} className="flex items-center gap-3 bg-[var(--color-bg-card)] border border-green-500/20 rounded-xl p-3 hover:border-[var(--color-accent)] transition-all group">
                          ...
                          <div className="flex-1 min-w-0"><p className="text-sm text-white font-serif group-hover:text-[var(--color-accent)] transition-colors">{c.chn?.replace(/[（(][^）)]*[）)]/g, "").trim() || c.eng}</p></div>
            ...
            <div className="flex gap-2 mb-3">
              <input value={addIng} onChange={e => setAddIng(e.target.value)}
                ...
                className="flex-1 bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-full px-4 py-2 text-sm text-white placeholder:text-[var(--color-text-muted)] outline-none focus:border-[var(--color-accent)]" />
            ...
            <div className="mt-10 pt-8 border-t border-[var(--color-border)]">
              <h3 className="text-sm text-white font-serif mb-2 flex items-center gap-2">🛠 调酒器具</h3>
        ...
        {activeTab === "playlists" && (
          <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            <h2 className="text-lg text-white font-serif mb-2 flex items-center gap-2"><ListPlus size={16} className="text-[var(--color-accent)]" /> 我的酒单</h2>
                  ...
                  <div key={pl.id} className="bg-[var(--color-bg-card)] border border-[var(--color-border)] rounded-xl p-5 group hover:border-[var(--color-accent)] transition-all">
                    <div className="flex items-start justify-between">
                      <Link to={`/playlist/${pl.id}`} className="flex-1 min-w-0">
                        <h3 className="text-base text-white font-serif group-hover:text-[var(--color-accent)] transition-colors">{pl.name}</h3>
        ...
        {activeTab === "notes" && (
          <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
            <h2 className="text-lg text-white font-serif mb-2 flex items-center gap-2"><PenLine size={16} className="text-[var(--color-accent)]" /> 我的笔记</h2>
